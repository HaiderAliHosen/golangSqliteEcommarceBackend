...
//CreatedProductPagedResponse ---
func CreatedProductPagedResponse(request *http.Request, products []models.Product, page, pageSize, count int, commentsCount []int) interface{} {
	var resources = make([]interface{}, len(products))
	for index, product := range products {
		resources[index] = CreateProductDto(&product, commentsCount[index])
	}
	return CreatePagedResponse(request, resources, "products", page, pageSize, count)
}
